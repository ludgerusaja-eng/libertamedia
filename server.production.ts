...
const uploadDir = path.resolve(process.env.UPLOAD_DIR || path.join(distPath, 'uploads'));
...
app.get('/api/articles', async (req, res) => {
  ...
  if (category && category !== 'Semua') { where.push('a.category = ?'); params.push(category); }
  if (q) { where.push('(a.title LIKE ? OR a.excerpt LIKE ? OR u.name LIKE ?)'); const term = `%${q}%`; params.push(term, term, term); }
  const whereSql = where.join(' AND ');
  const [countRows] = await pool.query<RowDataPacket[]>(`SELECT COUNT(*) AS total FROM articles a LEFT JOIN users u ON u.id = a.author_id WHERE ${whereSql}`, params);
  const total = Number(countRows[0]?.total || 0);
  const [rows] = await pool.query<RowDataPacket[]>(`SELECT a.*, u.name AS author_name, u.email AS author_email, u.role AS author_role FROM articles a LEFT JOIN users u ON u.id = a.author_id WHERE ${whereSql} ORDER BY a.published_at DESC, a.created_at DESC LIMIT ? OFFSET ?`, [...params, limit, offset]);
  const articles = rows.map(row => ({ ...row, id: String(row.id) }));
  const data = await Promise.all(articles.map(async row => repository.findArticle(String(row.id))));
  res.json({ success: true, total, pagination: { page, limit, total, totalPages: Math.ceil(total / limit) }, data: data.filter(Boolean) });
});
app.get('/api/articles/:id', async (req, res) => {
  const article = await repository.findArticle(req.params.id);
  if (!article || article.status !== 'PUBLISHED') return res.status(404).json({ success: false, message: 'Artikel tidak ditemukan' });
...
app.post('/api/articles/:id/comments', rateLimit(clientKey, 10, 60 * 60), async (req, res) => {
  ...
  const author = sanitize(req.body?.author).slice(0, 160); const content = sanitize(req.body?.content).slice(0, 5000);
...
app.patch('/api/comments/:id', requireAuth, permission('comment:moderate'), async (req, res) => {
  ...
  const [result] = await pool.query<any>(`UPDATE comments SET status=?, moderated_at=NOW(), moderated_by=? WHERE id=?`, [status, (req as any).user.userId, req.params.id]);
  if (!result.affectedRows) return res.status(404).json({ success: false, message: 'Komentar tidak ditemukan.' });
...
app.post('/api/submissions', rateLimit(clientKey, 5, 60 * 60), async (req, res) => {
  const body = req.body || {};
  const title = sanitize(body.title).slice(0, 255); const content = sanitize(body.content).slice(0, 50000); const authorName = sanitize(body.authorName).slice(0, 160);
  if (!title || !content || !authorName) return res.status(400).json({ success: false, message: 'Judul, nama penulis, dan isi wajib diisi.' });
  const id = `sub-${Date.now()}-${crypto.randomBytes(3).toString('hex')}`;
  await pool.query(`INSERT INTO submissions (id, title, category, author_name, email, institution, content) VALUES (?, ?, ?, ?, ?, ?, ?)`, [id, title, sanitize(body.category || 'Opini').slice(0, 100), authorName, sanitize(body.authorEmail || '').slice(0, 190), sanitize(body.authorOrg || '').slice(0, 190), content]);
  res.status(201).json({ success: true, data: { id, title, authorName, submittedAt: new Date().toISOString(), status: 'SUBMITTED' } });
});
app.post('/api/submissions/:id/publish', requireAuth, permission('submission:publish'), async (req, res) => {
  const connection = await pool.getConnection();
  try {
    await connection.beginTransaction();
    const [rows] = await connection.query<RowDataPacket[]>(`SELECT * FROM submissions WHERE id=? FOR UPDATE`, [req.params.id]);
    if (!rows.length) { await connection.rollback(); return res.status(404).json({ success:false, message:'Naskah warga tidak ditemukan.' }); }
    const row = rows[0];
    if (!['APPROVED','EDITING','FACT_CHECK','SCREENING'].includes(String(row.status)) && String(row.status) !== 'SUBMITTED') { await connection.rollback(); return res.status(409).json({ success:false, message:'Status naskah belum dapat diterbitkan.' }); }
    const id = `art-${Date.now()}-${crypto.randomBytes(3).toString('hex')}`;
    const title = sanitize(row.title); const slug = `${title.toLowerCase().replace(/[^a-z0-9\s-]/g, '').trim().replace(/\s+/g, '-').slice(0, 210)}-${Date.now().toString().slice(-6)}`;
    ...
    await connection.query(`UPDATE submissions SET status='PUBLISHED', reviewed_at=NOW(), reviewed_by=? WHERE id=?`, [(req as any).user.userId, req.params.id]);
    await connection.commit();
    res.status(201).json({ success:true, data: await repository.findArticle(id) });
  } catch (error) {
    await connection.rollback(); console.error('[submission-publish]', error); res.status(500).json({ success:false, message:'Gagal menerbitkan naskah.' });
  } finally { connection.release(); }
});
app.delete('/api/submissions/:id', requireAuth, permission('submission:review'), async (req, res) => { const [result] = await pool.query<any>(`UPDATE submissions SET status='REJECTED', reviewed_at=NOW(), reviewed_by=? WHERE id=?`, [(req as any).user.userId, req.params.id]); if (!result.affectedRows) return res.status(404).json({ success:false, message:'Naskah tidak ditemukan.' }); res.json({ success: true }); });
...
app.post('/api/settings', requireAuth, permission('settings:manage'), async (req, res) => { await pool.query(`INSERT INTO site_settings (id,data) VALUES (1,?) ON DUPLICATE KEY UPDATE data=VALUES(data), updated_at=NOW()`, [JSON.stringify(req.body)]); await repository.audit({ actorUserId: (req as any).user.userId, action:'SETTINGS_UPDATED', entityType:'site_settings', entityId:'1', ipAddress:req.ip, userAgent:req.get('user-agent') }); res.json({ success: true, data: req.body }); });
...
app.post('/api/pages', requireAuth, permission('settings:manage'), async (req, res) => { const id = String(req.body.id || `page-${Date.now()}`); const slug=sanitize(req.body.slug).slice(0,255); const title=sanitize(req.body.title).slice(0,255); const content=sanitize(req.body.content).slice(0,100000); if(!slug||!title) return res.status(400).json({success:false,message:'Slug dan judul wajib diisi.'}); await pool.query(`INSERT INTO pages (id,slug,title,content) VALUES (?,?,?,?)`, [id,slug,title,content]); await repository.audit({actorUserId:(req as any).user.userId,action:'PAGE_CREATED',entityType:'page',entityId:id,ipAddress:req.ip,userAgent:req.get('user-agent')}); res.status(201).json({ success:true, data:{ id, slug, title, content } }); });
app.put('/api/pages/:id', requireAuth, permission('settings:manage'), async (req, res) => { const slug=sanitize(req.body.slug).slice(0,255); const title=sanitize(req.body.title).slice(0,255); const content=sanitize(req.body.content).slice(0,100000); const [result]=await pool.query<any>(`UPDATE pages SET slug=?,title=?,content=? WHERE id=?`, [slug,title,content,req.params.id]); if(!result.affectedRows) return res.status(404).json({success:false,message:'Halaman tidak ditemukan.'}); await repository.audit({actorUserId:(req as any).user.userId,action:'PAGE_UPDATED',entityType:'page',entityId:req.params.id,ipAddress:req.ip,userAgent:req.get('user-agent')}); res.json({ success:true, data:{ id:req.params.id, slug,title,content } }); });
app.delete('/api/pages/:id', requireAuth, permission('settings:manage'), async (req, res) => { const [result]=await pool.query<any>(`DELETE FROM pages WHERE id=?`, [req.params.id]); if(!result.affectedRows) return res.status(404).json({success:false,message:'Halaman tidak ditemukan.'}); await repository.audit({actorUserId:(req as any).user.userId,action:'PAGE_DELETED',entityType:'page',entityId:req.params.id,ipAddress:req.ip,userAgent:req.get('user-agent')}); res.json({success:true}); });

app.get('/api/stats', requireAuth, permission('dashboard:view'), async (_req, res) => {
...
app.post('/api/upload', requireAuth, permission('media:upload'), async (req, res) => {
  try {
    ...
    const fileName = `${id}.webp`; const target = path.join(uploadDir, fileName); const buffer = Buffer.from(match[2], 'base64');
    if (buffer.length > 10 * 1024 * 1024) return res.status(413).json({ success:false, message:'Ukuran gambar maksimal 10MB.' });
    const metadata = await sharp(buffer).rotate().resize({ width: 2400, withoutEnlargement: true }).webp({ quality: 82 }).toFile(target);
    const relativePath = `/uploads/${fileName}`;
    await pool.query(`INSERT INTO media (id,uploaded_by,original_name,mime_type,storage_path,width,height,size_bytes) VALUES (?,?,?,?,?,?,?,?)`, [id,(req as any).user.userId,`${id}.webp`,'image/webp',relativePath,metadata.width || null,metadata.height || null,buffer.length]);
    res.json({ success:true, url:relativePath });
  } catch (error) {
    console.error('[upload]', error); res.status(400).json({ success:false, message:'File gambar tidak valid atau gagal diproses.' });
  }
});

app.get('/uploads/:file', (req, res) => {
  const file = path.basename(req.params.file);
  if (!/^[a-zA-Z0-9._-]+\.webp$/.test(file)) return res.status(404).end();
  const target = path.join(uploadDir, file);
  if (!fs.existsSync(target)) return res.status(404).end();
  res.type('image/webp').setHeader('Cache-Control','public, max-age=86400').sendFile(target);
});

app.use(express.static(distPath, { index: 'index.html', maxAge: '1h' }));
app.get('*', (req, res, next) => { if (req.path.startsWith('/api/') || req.path.startsWith('/uploads/')) return next(); res.sendFile(path.join(distPath, 'index.html')); });
