...
export class ProductionRepository {
  ...
  constructor() {
    ...
    for (const key of required) {
      if (!process.env[key]) {
        throw new Error(`[ProductionRepository] Missing required environment variable: ${key}`);
      }
    }
  ...
  async revokeSession(sessionId: string): Promise<void> {
    await this.pool.query(`UPDATE sessions SET revoked_at = NOW() WHERE id = ?`, [sessionId]);
  }
  ...
  async saveArticle(input: ArticleWriteInput, actorUserId?: number | null): Promise<void> {
    ...
    try {
      ...
      const [existing] = await connection.query<RowDataPacket[]>(
        `SELECT title, excerpt, content FROM articles WHERE id = ? FOR UPDATE`,
        [input.id]
      );
      const revisionNumber = existing.length
        ? Number((await connection.query<RowDataPacket[]>(
            `SELECT COALESCE(MAX(revision_number), 0) AS revision FROM article_revisions WHERE article_id = ?`,
            [input.id]
          ))[0][0].revision) + 1
        : 1;
      ...
      await connection.query<ResultSetHeader>(
        `INSERT INTO articles
          (id, slug, title, excerpt, content, category, pillar, image_url, image_caption,
           author_id, status, scheduled_at, published_at, is_hero, is_editor_choice, is_trending)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
         ON DUPLICATE KEY UPDATE
           ...
           image_caption=VALUES(image_caption), author_id=VALUES(author_id), status=VALUES(status),
           scheduled_at=VALUES(scheduled_at), published_at=VALUES(published_at),
           is_hero=VALUES(is_hero), is_editor_choice=VALUES(is_editor_choice), is_trending=VALUES(is_trending)`,
        [
          input.id,
          input.slug,
          input.title,
          ...
          Boolean(input.isHero),
          Boolean(input.isEditorChoice),
      ...
      await connection.query(
        `INSERT INTO article_revisions
          (article_id, editor_id, revision_number, title, excerpt, content, change_summary)
         VALUES (?, ?, ?, ?, ?, ?, ?)`,
        [input.id, actorUserId ?? null, revisionNumber, input.title, input.excerpt || '', input.content, 'Article saved']
      );
  ...
  async audit(context: AuditContext): Promise<void> {
    await this.pool.query(
      `INSERT INTO audit_logs (actor_user_id, action, entity_type, entity_id, ip_address, metadata)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [context.actorUserId ?? null, context.action, context.entityType, context.entityId ?? null, context.ipAddress ?? null, JSON.stringify(context.metadata || {})]
    );
  }
  ...
  private toArticle(row: RowDataPacket): Article {
    return {
      ...
      author: {
        name: String(row.author_name || 'Redaksi LIBERTAMEDIA'),
        email: String(row.author_email || ''),
        role: String(row.author_role || 'REPORTER'),
        avatar: ''
      },
